...
static std::vector<Keypoint> FAST(const int w, const int h, const unsigned char* grayImageData,
    int N = 9, float threshold  = 0.15f, int   nmsWindow = 2)
{
    ...
    for (int y = 3; y < h - 3; ++y)
    {
        for (int x = 3; x < w - 3; ++x)
        {
            ...
            for (int i = 0; i < 16 * 2; ++i)
            {
                ...
                if (v >= Ip + t) { ++b_cnt; b_max = std::max(b_max, b_cnt); } else b_cnt = 0;
                if (v <= Ip - t) { ++d_cnt; d_max = std::max(d_max, d_cnt); } else d_cnt = 0;
            }
...
inline int computeHarrisResponse(
    const unsigned char* src, const int width, const int height,
    const int px, const int py, const int blockSize, const double k,
...
//   rgbImageData  : pointer to a contiguous buffer of size w * h * 3 bytes (RGB, row-major order) 
    ...
    const int maxPoints, float* outPointer)
{
    ...
    struct Scored { int x, y; float score, angle; };
    ...
    for (const auto& kp : kps)
    {
        ...
        if (computeHarrisResponse(gray.data(), w, h, kp.x, kp.y, 3, 0.04, score, angle) == 0 && score > 0.f) 
        {
