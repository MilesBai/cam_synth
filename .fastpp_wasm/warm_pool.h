...
// in my testing, this gives more legible call stacks but doesn't noticeably
// help with performance or prevent any Wasm-specific correctness/robustness
// issues, so it's "not essential" AFAIK
...
class WarmPool {
 public:
  ...
  // note that if you use warm(true) and run under Valgrind, you'll need to run
  // `valgrind --fair-sched=yes` or the code will get stack busy-waiting on
  // variables set by other threads, which Valgrind by default doesn't schedule
  // until the current thread yields
  ...
  void parallel_for(size_t start, size_t end, Func&& func) {
    ...
    if (nested_.fetch_add(1, std::memory_order_relaxed) > 0 ||
        ready_.load(std::memory_order_relaxed) < num_threads) {
    ...
    while (current_value < expected) {
      if (warm_) {
        ...
        emscripten_futex_wait(reinterpret_cast<uint32_t*>(&workers_waiting_),
                              current_value, -1);
#else
  ...
  double main_iter_share() const {
    return double(main_done_) / (main_done_ + worker_done_);
  }
  void reset_stats() {
    main_done_ = 0;
    worker_done_ = 0;
 ...
 private:
  void spin(int& warm_iter) {
    ...
    // if we're warm for a while and nothing happens, it's likely that the
    // caller forgot to cool us down, so let's do it ourselves
...
#ifdef BUILTIN_EMSCRIPTEN_NOTIFY_WAIT
          emscripten_futex_wait(reinterpret_cast<uint32_t*>(&work_id_), last_id,
                                -1);
#else
  ...
  // these are just to see how much work is offloaded by the workers from the
  // main thread
  int main_done_ = false;
  int worker_done_ = false;

  std::atomic<int> nested_;  // nested parallel_fors are simply serialized in
                             // this implementation. this also serializes
  // all but the first parallel_for submitted concurrently from several
  // different threads.
  bool warm_ = false;  // both the main thread and the workers busy-wait instead
                       // of using wait() when the pool is "warm"
  int cool_after_ = 0;

  std::vector<std::thread> threads_;
  std::atomic<int>
      ready_;  // we use this to see how many threads have started (on Wasm, it
               // takes some time for them to start...)
  //- as long as the entire pool hasn't started, we serialize parallel_fors, or
  // we'd get a large slowdown instead of the decent speedup expected by the
